...
import cn.sirenia.mybatis.anno.AutoBaseResultMap;
import cn.sirenia.mybatis.util.Beans;

/**
 * @author Clinton Begin
 */
public class MyXMLMapperBuilder extends BaseBuilder {
  ...
  @Deprecated
  public MyXMLMapperBuilder(Reader reader, Configuration configuration, String resource, Map<String, XNode> sqlFragments, String namespace) {
    this(reader, configuration, resource, sqlFragments);
    this.builderAssistant.setCurrentNamespace(namespace);
  }

  @Deprecated
  public MyXMLMapperBuilder(Reader reader, Configuration configuration, String resource, Map<String, XNode> sqlFragments) {
  ...
  private MyXMLMapperBuilder(XPathParser parser, Configuration configuration, String resource, Map<String, XNode> sqlFragments) {
    super(configuration);
  ...
  private void configurationElement(XNode context) {
    try {
      ...
      List<XNode> resultMapNodes = context.evalNodes("/mapper/resultMap");
      addBaseResultMapElement(namespace,resultMapNodes);
      resultMapElements(resultMapNodes);
  ...
  private void addBaseResultMapElement(String namespace, List<XNode> resultMapNodes) throws ClassNotFoundException {
	  ...
	  if(genericInterfaces!=null&&genericInterfaces.length>0){
		  for(int i=0;i<genericInterfaces.length;i++){
			  ...
			  if(anno!=null && typeName.contains("BaseMapper")){
				  ...
				  XNode node = mapModelToBaseResultMapNode(modelClass);
				  resultMapNodes.add(node);
			  }
		  }
	  }
  }

private XNode mapModelToBaseResultMapNode(Class<?> modelClass) {
	Field[] fields = modelClass.getDeclaredFields();
	StringBuilder sb = new StringBuilder();
	sb.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
	sb.append(String.format("<resultMap id=\"BaseResultMap\" type=\"%s\">", modelClass.getName()));
	if(fields!=null&&fields.length>0){
		for(int i=0;i<fields.length;i++){
			Class<?> fieldType = fields[i].getType();
			String jdbcType = javaTypeTojdbcType(fieldType);
			if(jdbcType==null){
				continue;
			}
			...
			if(fields[i].getName().equals("id")){
				...
				sb.append(String.format("<result column=\"%s\" jdbcType=\"%s\" property=\"%s\" />",columnName,jdbcType,propertyName));
			}
		}
	}
	sb.append("</resultMap>");
	XNode node = new XPathParser(sb.toString()).evalNode("/resultMap");
	return node;
}
private String javaTypeTojdbcType(Class<?> fieldType) {
	...
	map.put("java.util.Date", "TIMESTAMP");
	map.put("java.math.BigDecimal", "NUMERIC");
	//map.put("[B","LONGVARBINARY");
	map.put("[B","VARBINARY");//byte[]
