...
* @version $Id: MapperScannerConfigurer.java 5179 2012-04-15 18:09:44Z eduardo.macarron $
*/
public class MyMapperScannerConfigurer implements BeanDefinitionRegistryPostProcessor, InitializingBean, ApplicationContextAware, BeanNameAware {
 ...
 private final class Scanner extends ClassPathBeanDefinitionScanner {
   ...
   protected void registerDefaultFilters() {
     ...
     if (MyMapperScannerConfigurer.this.annotationClass != null) {
       addIncludeFilter(new AnnotationTypeFilter(MyMapperScannerConfigurer.this.annotationClass));
       acceptAllInterfaces = false;
     }

     // override AssignableTypeFilter to ignore matches on the actual marker interface
     if (MyMapperScannerConfigurer.this.markerInterface != null) {
       addIncludeFilter(new AssignableTypeFilter(MyMapperScannerConfigurer.this.markerInterface) {
         @Override
   ...
   protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
     ...
     if (beanDefinitions.isEmpty()) {
       logger.warn("No MyBatis mapper was found in '" + MyMapperScannerConfigurer.this.basePackage
       ...
       for (BeanDefinitionHolder holder : beanDefinitions) {
         ...
         definition.getPropertyValues().add("addToConfig", MyMapperScannerConfigurer.this.addToConfig);

         boolean explicitFactoryUsed = false;
         if (StringUtils.hasLength(MyMapperScannerConfigurer.this.sqlSessionFactoryBeanName)) {
           definition.getPropertyValues().add("sqlSessionFactory", new RuntimeBeanReference(MyMapperScannerConfigurer.this.sqlSessionFactoryBeanName));
           explicitFactoryUsed = true;
         } else if (MyMapperScannerConfigurer.this.sqlSessionFactory != null) {
           definition.getPropertyValues().add("sqlSessionFactory", MyMapperScannerConfigurer.this.sqlSessionFactory);
           explicitFactoryUsed = true;
         }

         if (StringUtils.hasLength(MyMapperScannerConfigurer.this.sqlSessionTemplateBeanName)) {
           if (explicitFactoryUsed) {
             logger.warn("Cannot use both: sqlSessionTemplate and sqlSessionFactory together. sqlSessionFactory is ignored.");
           }
           definition.getPropertyValues().add("sqlSessionTemplate", new RuntimeBeanReference(MyMapperScannerConfigurer.this.sqlSessionTemplateBeanName));
           definition.getPropertyValues().add("sqlSessionFactory", null);
         } else if (MyMapperScannerConfigurer.this.sqlSessionTemplate != null) {
           if (explicitFactoryUsed) {
             logger.warn("Cannot use both: sqlSessionTemplate and sqlSessionFactory together. sqlSessionFactory is ignored.");
           }
           definition.getPropertyValues().add("sqlSessionTemplate", MyMapperScannerConfigurer.this.sqlSessionTemplate);
           definition.getPropertyValues().add("sqlSessionFactory", null);
         }
       }
     }
